...
import { getAllThoughts, getThoughtBySlug, getRecentThoughts } from '../thoughts'
		...
		it('should sort thoughts by date (newest first)', () => {
			...
			if (thoughts.length > 1) {
				for (let i = 0; i < thoughts.length - 1; i++) {
					expect(thoughts[i]?.dateCreated.getTime()).toBeGreaterThanOrEqual(
	...
	describe('getThoughtBySlug', () => {
		it('should find existing thought by year and slug', () => {
			...
			if (allThoughts.length > 0) {
				...
				if (firstThought) {
					const found = getThoughtBySlug(firstThought.year, firstThought.slug)
